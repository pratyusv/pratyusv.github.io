...
* On average each users sends *50 messages* daily
...
<p> Concepts </p>
1. We can maintian **websockets or Long polling** between user devices and servers to **push** notifications.
...
When connection breaks between user and websocket and the user re-establishes connection with other handler, the information is updated to Redis.
...
**Message Service**
  * WebSocket handler talks to message service to store/retrive messages. 
  * It exposes APIs to retrieve messages
  * Message Service sits on top of database
...
1. UserA connects to a WebSocket Handler on a port. This information is sent to *Websocket Manager* which stores it in its *Redis Cache*. This mapping *[UserID: WebSocket]* can be later used by other WebSocket Handler who want to send message to UserA. UserA is assigned port: 10562.
2. UserA sends message to WebSocket Handler intended to UserB
3. If the websocket handler does not know which webserver is connected to userB, it contacts webSocket Manager for the infomration. Websocket Manager maintains the information of all the connected users in its Redis Cache. In parallel the message is also sent to Message service which stores it in the Cassandra database.
4. Once the Websocket handler receives the socket address of userB, it forwards the message to that websocket handler.

**User Online**
1. If the user is online an ack is recieved from UserB and forwarded to the UserA as well as updated to the database

**User Offline**
1. If UserB is offline, the message gets stored in database with status set as *not delivered*. 
2. When UserB connectes to any webosocket handler, the handler contacts messaging service to check if there are any pending messages for UserB.
...
Group Message handler sits on top of kafka queue and is responsible for sending messages to all the members of group.
...
For a media messsaage, ideally it is compressed and encrypted at the sender's end. Similarly decryption and uncompression happens at the receiver's end. When a user sends a media message following steps are taken:
...
The media is stored in S3 database. Depending upon the usage it may be stored to CDN as well.
